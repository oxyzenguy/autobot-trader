...
from strategy_logic import calculate_new_buy_prices
...
class BacktestConfig:
    ...
    sell_profit_margin: float = 1.002
...
def run_backtest(df: pd.DataFrame):
    ...
    print("\n" + "="*50)
    print(f"백테스트 시작 (1 Unit = {UNIT_KRW:,}원)")
    ...
    if not df.empty:
        ...
        current_cycle['entries'].append({
            'type': 'market_buy', 'price': start_price, 
            'volume': buy_volume, 'cost': buy_cost, 'time': df.index[0]
        })
        ...
        new_buy_prices = calculate_new_buy_prices(avg_buy_price, existing_orders=None)
        simulated_open_buy_orders = []
        for price in new_buy_prices:
            buy_volume_for_order = UNIT_KRW / price
            simulated_open_buy_orders.append({'price': price, 'volume': buy_volume_for_order})
    ...
    for index, candle in df.iloc[1:].iterrows():
        ...
        if current_cycle and simulated_open_sell_orders and high_price >= simulated_open_sell_orders[0]['price']:
            ...
            current_cycle['entries'].append({
                'type': 'market_buy_reentry', 'price': buy_price,
                'volume': buy_volume, 'cost': buy_cost, 'time': current_time
            })
            current_cycle['total_cost'] += buy_cost
            current_cycle['total_volume'] += buy_volume
            total_actions += 1
        ...
        if filled_buy_orders and current_cycle:
            ...
            for order in filled_buy_orders:
                ...
                current_cycle['entries'].append({
                    'type': 'limit_buy', 'price': buy_price,
                    'volume': buy_volume, 'cost': buy_cost, 'time': current_time
                })
                current_cycle['total_cost'] += buy_cost
                current_cycle['total_volume'] += buy_volume
            ...
            if filled_count > 0 and current_cycle['total_volume'] > 0:
                ...
                new_buy_prices = calculate_new_buy_prices(avg_buy_price, existing_orders=simulated_open_buy_orders)
